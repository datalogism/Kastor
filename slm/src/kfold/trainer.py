...
from kfold.ensemble import EnsembleLightningModule
...
class KFoldTrainer(Trainer):
    ...
    def __init__(self, num_folds: int = 5, shuffle: bool = False, stratified: bool = False,project="cross_validation",group="test",callbacks_conf={}, *args, **kwargs) -> None:
        ...
        self.callback_conf=callbacks_conf
        self.project=project
        self.group=group
        # Intialize rest of the trainer
        super().__init__(*args, **kwargs)
        self._version = self.logger.version
        #self.callback_init=self.callbacks
        

    def init_newWBLogger(self,nb_fold):
        print("XXXXXXXXXXXXXXXXXXXXXXX INIT WBLog")
        wandb.finish()
        self.logger = WandbLogger(project = self.project,
            name = self.group+"_"+nb_fold,group=self.group)

    # def init_callbackstore(self,nb_fold):
    #     print("XXXXXXXXXXXXXXXXXXXXXXX INIT CALLBACK STORE")
    #     callbacks_store = []
    #     print(">",len(self.callbacks))
    #     if self.callback_conf["apply_early_stopping"]:
    #         callbacks_store.append(
    #             EarlyStopping(
    #                 monitor=self.callback_conf["monitor"],
    #                 mode=self.callback_conf["mode"],
    #                 patience=self.callback_conf["patience"]
    #             )
    #         )
    ...
    #             monitor=self.callback_conf["monitor"],
    #             # monitor=None,
    #             dirpath=f'experiments/'+self.group+"_"+nb_fold,
    #             save_top_k=self.callback_conf["save_top_k"],
    #             verbose=True,
    #             save_last=True,
    #             mode=self.callback_conf["mode"]
    #         )
    ...
    #     #callbacks_store.append(GenerateTextSamplesCallback(self.callback_conf["samples_interval"]))
    ...
    def cross_validate(
        ...
        ##### STRANGE FIX
        if(self.fit_loop.max_epochs):
            nb_max_epoch=self.fit_loop.max_epochs
        else:
            nb_max_epoch=None
        if(self.fit_loop.max_epochs):
            max_steps=self.fit_loop.epoch_loop.max_steps
        else:
            max_steps=None

        print("================>LOOOP PARAMS")
        print(self.fit_loop.max_epochs)
        print(self.fit_loop.max_steps)
        #print(self.fit_loop.num_train_epochs)
        #print(self.wait)
        #print(self.stopped_epoch)
        # run cross validation
        results, paths = [], []
      
        for i in range(self.num_folds):

            self.init_newWBLogger(str(i))
            #self.init_callbackstore(str(i))
            ...
            state=self.state
            print("-------------->",state)
#https://github.com/Lightning-AI/lightning/issues/12409
            if(nb_max_epoch):
                print("+nb_max_epoch")
                self.fit_loop.max_epochs += nb_max_epoch
            if(max_steps):
                print("+max_steps")
                self.fit_loop.epoch_loop.max_steps += max_steps
