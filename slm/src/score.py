...
def re_score_withShapeWithObjandFilter(pred_relations, gt_relations, format_, syntax_conf, shape, grammar, input_dict):
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>RESCORE OBJ NEW")
    print("#")
    print("#")
    print("#")
    print("#")
    ...
    for p_sent, g_sent in zip(pred_relations_clean, gt_relations_clean):
        ...
        if (format_ == "turtleS"):
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>> ENCODEOBJS")
            p_sent = scr_fct.encodeTurtleS_obj(p_sent)
            g_sent = scr_fct.encodeTurtleS_obj(g_sent)
        ...
        if (parsed_pred == False):
            ...
            list_rel_pred=scr_fct.filterPredList(list_rel_pred,input_dict)
        print("==============")
        print(list_rel_pred)
        print("-----------")
        print(list_rel_gt)
        ...
        if (parsed_gt and isinstance(list_rel_gt, list) and len(list_rel_gt) > 0):
            ...
            subj_clean_gt = scr_fct.uncodeurl(subj_gt.replace("dbr:", "").replace(":", "").strip())
            ...
            if (parsed_pred and isinstance(list_rel_pred, list) and len(list_rel_pred) > 0):
                print(">>>>>>>>>>>>>>>>>>>>>>>>>>> INSIDE")
                print(parsed_pred, "in list_rel_pred")
                print("list_rel_pred>",list_rel_pred)
                ## CHECK TYPE
                pred_type = []
               ...
               # print(">>>>>>>>>>>1")
                ...
                #relation_realised=list(set(relation_realised+set_rel_gold+set_rel_pred))
                #print(">>>>>>>>>>>2")
                ...
                if (valid_rdf == True):
                    ...
                    notvalid_examples.append({"ent_uri": subj_clean_gt, "p_pred": str(set_rel_pred), "g_pred": str(set_rel_gold)})
                    print("NOT VALID")

                print(">>>>>>>>>>>4")
                subj_pred = str(list_rel_pred[0][0])
                subj_pred = subj_pred.replace("http://dbpedia.org/resource/", "")
                subj_clean_pred = scr_fct.uncodeurl(subj_pred.replace("dbr:", "").replace(":", "").strip())

                print(">list_rel_pred",list_rel_pred)
                for triple in list_rel_pred:
                    print("triple>len",len(triple))

                    if (len(triple) == 3 and "]" not in triple):
                        rel = str(triple[1])
                        val = str(triple[2]).strip()
                    ...
                    if (rela in dict_rel_gt.keys()):
                        ...
                        if (rela in dict_rel_pred.keys()):
                            for val in dict_rel_pred[rela]:
                                ...
                                print(">>>>>>>>>> IN DICT ? ",val)
                                if (str(val).strip() in dict_rel_gt[rela]):
                                    print("tp")
                                    scores[rela]["tp"] += 1
                                    ...
                                    if (input_dict and subj_clean_pred in input_dict.keys()):
                                        abstract = input_dict[subj_clean_pred]
                                        find_in_abs = scr_fct.find_in_abstractWithObj(abstract, rela, val,
                                                                               relations_map[rela]["dt"])
                                        temp = {"ent_uri": subj_clean_pred, "pred": rela, "val_gold": dict_rel_gt[rela][0],
                            ...
                            temp = {"type": "FN", "ent_uri": subj_clean_pred, "pred": rela, "val_gold": val, "val_pred": "",
                                    "abs": "", "found": False}

                            if (input_dict and subj_clean_pred in input_dict.keys()):
                                print("FIND IN ABS>",val,">rel",rela)
                                abstract = input_dict[subj_clean_pred]
                                temp["abs"] = abstract

                                temp["found"] = scr_fct.find_in_abstractWithObj(temp["abs"], rela, val,
                                                                         relations_map[rela]["dt"])
                            else:
                                print("NOT FOUND >", subj_clean_pred)
                                #print(list(input_dict.keys()))
                            to_inspect.append(temp)


                    elif (rela in dict_rel_pred.keys()):
                        print("in dict_rel_pred")
                        temp = {"type": "FP", "ent_uri": subj_clean_pred, "pred": rela, "val_gold": "", "val_pred": "",
                                    "abs": "", "found": False}
                        ...
                        if (input_dict and subj_clean_pred in input_dict.keys()):
                            abstract = input_dict[subj_clean_pred]
                            print("FIND IN ABS>", val, ">rel", rela)
                            find_in_abs = scr_fct.find_in_abstractWithObj(abstract, rela, val, relations_map[rela]["dt"])
                            temp = {"ent_uri": subj_clean_pred, "pred": rela, "val_gold": "", "val_pred": val,
                ...
                if (subj_clean_gt != "" and subj_clean_gt == subj_clean_pred):
                    n_subj_ok += 1
                else:
                    is_examples.append({"subj_gt": subj_clean_gt, "subj_pred": subj_clean_pred})
                if (parsed_pred):
                    n_parsed += 1
            else:
    ...
    if ((tp + tn + fp + fn) != 0):
        ...
        scores["ALL"]["FnRate"] = 0
        scores["ALL"]["FpRate"] = 0
    scores["ALL"]["Macro_f1"] = np.mean([scores[ent_type]["f1"] for ent_type in relations_all if scores[ent_type]["nb_gt"]>0])
    scores["ALL"]["Macro_p"] = np.mean([scores[ent_type]["p"] for ent_type in relations_all if scores[ent_type]["nb_gt"]>0])
    scores["ALL"]["Macro_r"] = np.mean([scores[ent_type]["r"] for ent_type in relations_all if scores[ent_type]["nb_gt"]>0])
...
def re_score_withShapeWithObj(pred_relations, gt_relations, format_, syntax_conf, shape, grammar, input_dict):
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>RESCORE OBJ")
    print(shape.serialize(format="ttl"))
    print("SHAPE OK")
    get_prop = """
...
def re_score_withShapeWithObjREBEL(pred_relations, gt_relations,  shape, input_dict):
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>RESCORE OBJ")
    print(shape.serialize(format="ttl"))
    print("SHAPE OK")
    get_prop = """
    ...
    n_typeEnt_ok = 0
    edit_dist = []
    pred_relations_clean = pred_relations
    gt_relations_clean = gt_relations
    ...
    for p_sent, g_sent in zip(pred_relations_clean, gt_relations_clean):
        ...
        subj_pred = ""
        print("BEFORE HER")
        list_rel_gt, parsed_gt = g_sent, True
        list_rel_pred, parsed_pred = p_sent, True
    ...
    callbacks = {"to_inspect": to_inspect, "is_examples": is_examples, "notvalid_examples": notvalid_examples,
                 "notparsed_examples": notparsed_examples}
    return scores, part_parsed, part_subj_ok, part_valid_s, part_valid_rdf, callbacks
def re_score_withShapeWithObjText2KG(pred_relations, gt_relations, format_, syntax_conf, shape, grammar, input_list):
    print(">>>>>>>>>>>>>>>>>>>>>>>>>>RESCORE OBJ")
    print(shape.serialize(format="ttl"))
    print("SHAPE OK")
    get_prop = """
    ...
    notvalid_examples = []
    notparsed_examples = []
    for p_sent, g_sent, abs in zip(pred_relations_clean, gt_relations_clean,input_list):
        p_sent = str(p_sent)
        g_sent = str(g_sent)
        if (format_ == "turtleS"):
            print(">>>>>>>>>>>>>>>>>>>>>>>>>>>>> ENCODEOBJS")
    ...
    if ((tp + tn + fp + fn) != 0):
        scores["ALL"]["Accuracy"] = (tp + tn) / (tp + tn + fp + fn)
        # scores["ALL"]["HalluRate"] =  (fp - dtp)  /(tp + tn + fp + fn)
        scores["ALL"]["FnRate"] = fn / (tp  + fp + fn)
        scores["ALL"]["FpRate"] = fp / (tp  + fp + fn)
        # scores["ALL"]["DiscoRate"] = dtp  /(tp + tn + fp + fn)
        # if((scores["ALL"]["DiscoRate"] + scores["ALL"]["FnRate"] + scores["ALL"]["HalluRate"])!=0):
        #     scores["ALL"]["KCP"] = (scores["ALL"]["DiscoRate"] - ( scores["ALL"]["FnRate"] + scores["ALL"]["HalluRate"] ) ) / (scores["ALL"]["DiscoRate"] + scores["ALL"]["FnRate"] + scores["ALL"]["HalluRate"])
        # else:
        ...
        if (parsed_gt and isinstance(list_rel_gt, list) and len(list_rel_gt) > 0):
            ...
            if (parsed_pred and isinstance(list_rel_pred, list) and len(list_rel_pred) > 0):
                ...
                if (valid_rdf == True):
                    ...
                    notvalid_examples.append({"ent_uri": subj_gt, "triples": str(list_rel_pred), "SHACL_repport": str(results_text)})
