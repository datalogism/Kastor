...
def getNbEntToDO(sparql_ep,search_ng,current_ng):
    ...
    query = "PREFIX dcat: <http://www.w3.org/ns/dcat#> select  (COUNT(DISTINCT ?s) as ?nb)  FROM <" + search_ng + "> WHERE  {   ?s dcat:resource_identifier  ?uid. FILTER NOT EXISTS { GRAPH  <" + current_ng + "> { ?s ?p ?o }  } } "
    print(query)
...
def get_SubjectsToRetrieveWithIDAndIdNS(sparql_ep, search_ng, id_ng, current_ng, limit):
    ...
    FROM <''' + search_ng + '''> WHERE  { 
       ...
       ?s <http://dbpedia.org/ontology/wikiPageRevisionID> ?id. FILTER NOT EXISTS { GRAPH  <''' + current_ng + '''> { ?s ?p ?o }  }
...
if __name__ == '__main__':
    ...
    wikipedia_agent="(https://datalogism.github.io/; celian.ringwald@inria.fr) Inria2"
    ...
    if args.shape_file_path:
        ...
        while to_do>0:
           ...
           sample=  get_SubjectsToRetrieveWithIDAndIdNS(sparql_ep,search_ng,ns_class_ids,current_ng,limit)
