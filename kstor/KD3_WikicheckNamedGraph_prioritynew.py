...
def get_NbEntitiesNGToDo(ng, found_ng, sparql_ep):
    ...
    sparql.setQuery(
        "SELECT (COUNT(DISTINCT ?s) as ?nb) FROM <" + ng + "> where {  ?s ?p ?o.  FILTER  EXISTS { GRAPH  <http://ns.inria.fr/kstor/wikinew_202004/> { ?s ?pi ?oi } }.   FILTER NOT EXISTS { GRAPH  <" + found_ng + "> { ?s ?p ?o }  }  }")
...
def get_NbEntitiesNG(ng, sparql_ep):
    ...
    sparql.setQuery(
        "PREFIX dbo: <http://dbpedia.org/ontology/>  PREFIX dcat: <http://www.w3.org/ns/dcat#>  SELECT (COUNT(DISTINCT ?s) as ?nb) from <" + ng + ">  where {  ?s ?p ?o.  FILTER  EXISTS { GRAPH  <http://ns.inria.fr/kstor/wikinew_202004/> { ?s ?pi ?oi } }. }")
    sparql.setReturnFormat(JSON)
    qres = sparql.query().convert()
...
if __name__ == '__main__':
    ...
    if args.shape_file_path and args.searchspace_namedgraph:
        ...
        nb_entities=get_NbEntitiesNG(search_ng,sparql_ep)
        ...
        to_do=get_NbEntitiesNGToDo(search_ng,found_ng,sparql_ep)

        while count_ent_checked!=to_do:
            print(">>>>>>>>>>>>>>>>>>>>>LOOP :" ,nb_loop,">",count_ent_checked,"/",to_do)
            sample=  cs.get_sampleNG_to_updateOnlyNew(search_ng,found_ng,sparql_ep,limit, offset)
