...
# -*- coding: utf-8 -*-
"""
1_KD-4_getExampleSpecificPatternsStats.py

This script analyzes RDF data against SHACL shapes to generate statistics about property usage
and class signature patterns. It processes shape files and queries a SPARQL endpoint to calculate
property frequencies and class signature distributions in the RDF data.

Key functionalities:
- Extract properties and their types from SHACL shapes
- Calculate property usage statistics in the target RDF graph
- Analyze class signature patterns and their frequencies
- Export results as JSON and CSV for further analysis

Usage:
    python 1_KD-4_getExampleSpecificPatternsStats.py -s <shape_file> -output <output_dir> -ng <named_graph>
"""
    ...
    if args.shape_file_path and args.output_dir:
        ...
        if (exist_stats == True):
            ...
            for class_sign in class_sign_all:
                print(class_sign)
                nb_entities = cs.get_ClassSignatureNb_NG(type_triples, list(class_sign), prop_focus2, found_abs,
                                                         sparql_ep)
                print(nb_entities)
