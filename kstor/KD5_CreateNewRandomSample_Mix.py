...
if __name__ == '__main__':
    ...
    parser.add_argument("-sz", "--size_sample", default=1210)
    ...
    if args.shape_file_path and args.size_sample:
        ...
        #existing_sample="http://ns.inria.fr/kstor/samples/WrittenWorkShapeInWild/abtract_md/only_old_sample_0"
        ...
        if(existing_sample==None):
           ...
           # new = "http://ns.inria.fr/kstor/samples/" + shape_name + "/abtract_md/sample_"
            ...
            while("http://ns.inria.fr/kstor/samples/" + shape_name + "/abtract_md/mix_sample_"+str(idx) in other_samples):
                idx+=1
            sample_ng="http://ns.inria.fr/kstor/samples/" + shape_name + "/abtract_md/mix_sample_"+str(idx)
        else:
       ...
       # done_list += ['United_States', 'Arem-arem', 'Bacon_Explosion', 'BLT', 'Indonesia', 'Tomato', 'Amatriciana_sauce', 'Italy', 'Bacon_sandwich', 'Dessert', 'Arrabbiata_sauce', 'Baked_Alaska', 'Bionico', 'Mexico', 'Celery']
        ## CLEAR SAMPLE
       ...
       # query="SELECT * FROM <"+sample_ng+"> {?s ?p ?o} LIMIT 10"
        ...
        while(size_sample!=SAMPLE_len):
            ...
            for idx, row in results.iterrows():
                ...
                if (uri.replace("http://dbpedia.org/resource/","").replace("https://dbpedia.org/resource/","") not in done_list
