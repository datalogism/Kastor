...
if __name__ == '__main__':
    ...
    if args.shape_file_path and args.size_sample:
        ...
        if(existing_sample==None):
            ...
            sample_ng="http://ns.inria.fr/kstor/samples/" + shape_name + "/abtract_md/only_old_sample_HARD_"
           # new = "http://ns.inria.fr/kstor/samples/" + shape_name + "/abtract_md/sample_"
            ...
            while("http://ns.inria.fr/kstor/samples/" + shape_name + "/abtract_md/only_old_sample_HARD_"+str(idx) in other_samples):
                idx+=1
            sample_ng="http://ns.inria.fr/kstor/samples/" + shape_name + "/abtract_md/only_old_sample_HARD_"+str(idx)
        else:
        ...
        list_relation_focus=["http://dbpedia.org/ontology/alias", "http://dbpedia.org/ontology/birthName",
                             "http://dbpedia.org/ontology/nationality", "http://dbpedia.org/ontology/deathPlace"]
        #
        ## CLEAR SAMPLE
        ...
        while(size_sample!=SAMPLE_len):
            ...
            for idx, row in results.iterrows():
                ...
                if(uri not in done_list
                    ...
                    res=ct.get_sparql_dataframe(sparql_ep,query)
                    found_op = []
                    if (len(list_relation_focus) > 0):
                        for index, row2 in res.iterrows():
                            if(row2["p"] in list_relation_focus):
                                found_op.append(row2["p"])
