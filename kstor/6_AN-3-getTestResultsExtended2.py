...
if __name__ == '__main__':
    ...
    args.wandb_project="GenLimit_Test"
    args.dir_save="/user/cringwal/home/Desktop/RESULTS_DATA/GenLimits/"
    ...
    if args.wandb_api_key and args.wandb_user and args.wandb_project and args.dir_save:
        ...
        for run in runs:
            ...
            if("test" in name_orig ):
                ...
                tempo["RDF_p"]=0
                tempo["R_PD"]=0
                tempo["align_score_FP"]=0
                tempo["align_score_FN"]=0
                align_score_FP=[]
                align_score_FN=[]
                artifacts=run.logged_artifacts()
                #if('To Inspect table' in data_summary.keys()):
                #    print(data_summary['To Inspect table']["artifact_path"])
                #   artifact =data_summary['To Inspect table']["path"]
                #   print(artifact)
                #   print("xxx")
                #   tab=get_table_data_from_url(artifact,api_key=args.wandb_api_key)
                #   print(tab)
                        ...
                        if(table):
                            ...
                            for idx,row in df0.iterrows():
                                ...
                                if (rel_type == "FN"):
                                    ...
                                    val = row['val_pred']
                                    error_type = "FP"
                                subj2 = uncodeurl(row["ent_uri"].replace("_", " ").replace(":", ""))
                                rel_2 = rel_name.replace("_", " ").replace(":", "")
                                val_2 = val
                                if ("date" in rel_name.lower()):
                                    try:
                                        val_2 = date.fromisoformat(val).strftime("%d %B %Y")
                                    except:
                                        val_2 = uncodeurl(val).replace("_", " ").replace(":", "")

                                else:
                                    val_2 = uncodeurl(val).replace("_", " ").replace(":", "")

                                temp_as = {"ent_uri": subj2, "prop": rel_2, "value": val_2, "abstract": row["abs"]}
                                if error_type == "FN":
                                    align_score_FN.append(llm_eval.getAlignScore(temp_as))
                                else:
                                    align_score_FP.append(llm_eval.getAlignScore(temp_as))
                            ...
                            if(len(align_score_FP)>0):
                                tempo["align_score_FP"] = sum(align_score_FP)/len(align_score_FP)
                            if(len(align_score_FN)>0):
                                tempo["align_score_FN"] = sum(align_score_FN)/len(align_score_FN)
                ...
                df.to_csv(save_dir + "Test_results_wb_BigXP_DT&OP_MORE_"+name_orig+"_x10.txt")
        print(tempo)
        df = pd.DataFrame.from_dict(data_results)
        df.to_csv(save_dir+"Test_results_wb_BigXP_DT&OP_MORE_x10.csv")
