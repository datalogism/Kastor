...
def get_RandomSample_NG_EntAbsMD(sparql_ep, abstract_ng, wikichecked_ng, class_id_ng, limit=10, offset=0):
...
if __name__ == '__main__':
    ...
    if args.shape_file_path and args.size_sample:
        ...
        if (existing_sample == None):
            ...
            while ("http://ns.inria.fr/kstor/samplesv3/" + shape_name + "/abtract_md/mix_sample_" + str(
                    idx) in other_samples):
                idx += 1
            sample_ng = "http://ns.inria.fr/kstor/samplesv3/" + shape_name + "/abtract_md/mix_sample_" + str(idx)
        else:
        ...
        while (size_sample != SAMPLE_len):
            ...
            results = get_RandomSample_NG_EntAbsMD(sparql_ep, abstract_ng, found_ng, class_id_ng, limit, offset)
            list_current_uri = []
            for idx, row in results.iterrows():
                ...
                if (uri not in done_list
                    ...
                    query = "PREFIX dbo: <http://dbpedia.org/ontology/>  select ?p ?o FROM <" + found_ng + "> { <" + \
                            row["s"] + "> ?p ?o. }"
                    ...
                    for index, row2 in res.iterrows():
                        ...
                        if (real_prop in type_prop.keys()):
                            ...
                            if ("date" in real_prop.lower() and "date" in type_prop[real_prop]):
                                print(val_look)
